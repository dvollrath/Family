...
foreach d in head_dum_patri_strong /// 
			 head_dum_any_desc ///
	...
	foreach c in control0 control1 control2 {
		...
		qui reghdfe `d' i.`x'##(i.head_age_group ``c'') [aweight = dhs_weight], absorb(cluster_id) cluster(cluster_id)	
		...
		qui tabulate survey if e(sample)==1
