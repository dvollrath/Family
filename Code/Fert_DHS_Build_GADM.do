...
//foreach folder in DHSKM61 { // test directory
	...
	foreach file of local fileList { // for each of those HR files (should only be one, this is just a safety)
		...
		do "./Code/Fert_DHS_Build_GADM_HH.do" // create hh variables to get summary data on
		...
		collapse (median) dhs_p50_head_ed_years = head_ed_years dhs_p50_head_age = head_age dhs_p50_hh_mem_dejure = hh_mem_dejure ///
				dhs_p50_hh_hect = hh_hect dhs_p50_hh_cattle_num = hh_cattle_num dhs_p50_hh_draft_num = hh_draft_num dhs_p50_hh_sheep_num = hh_sheep_num ///
			(mean) dhs_hh_flush = hh_flush dhs_hh_elec = hh_elec dhs_hh_tv = hh_tv dhs_hh_frig = hh_frig dhs_hh_floor = hh_floor ///
					dhs_hh_land = hh_land dhs_hh_cattle_dum = hh_cattle_dum dhs_hh_draft_dum = hh_draft_dum dhs_hh_sheep_dum = hh_sheep_dum dhs_hh_bank = hh_bank dhs_hh_urban = hh_urban ///
			(p10) dhs_p10_head_ed_years = head_ed_years dhs_p10_head_age = head_age dhs_p10_hh_mem_dejure = hh_mem_dejure  ///
				dhs_p10_hh_hect = hh_hect dhs_p10_hh_cattle_num = hh_cattle_num dhs_p10_hh_draft_num = hh_draft_num pdhs_10_hh_sheep_num = hh_sheep_num ///
			(p90) dhs_p90_head_ed_years = head_ed_years dhs_p90_head_age = head_age dhs_p90_hh_mem_dejure = hh_mem_dejure  ///
				dhs_p90_hh_hect = hh_hect dhs_p90_hh_cattle_num = hh_cattle_num dhs_p90_hh_draft_num = hh_draft_num dhs_p90_hh_sheep_num = hh_sheep_num ///
		...
		if r(N)>0 { // only save if count is above 0, to avoid blank datasets
			qui save "./Work/`folder'-gadm-hh.dta", replace
		} // end if
...
local fileList : dir "./Work/" files "DHS????-gadm-hh.dta" // get list of all file name

clear
save "./Work/DHS-all-gadm-hh.dta", emptyok replace // create empty dataset
...
save "./Work/DHS-all-gadm-hh.dta", replace // save the base file

//save "~/Dropbox/project/crops/data/dhs/DHS-all-gadm-hh.dta", replace
save "~/Dropbox/project/crops/work/DHS-all-gadm-hh.dta", replace
